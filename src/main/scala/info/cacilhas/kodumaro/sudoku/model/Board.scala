...
class Board private(val header: String = "% started empty") {
  ...
  def update(x: Int, y: Int, value: Int): Unit = Future(blocking {
    Using(updateMutex) { mutex =>
      mutex.acquire()
      apply(x, y) filter {_.value != value} foreach { cell =>
        if (cell.tap {_.value = value}.value == value) updater upgrade (x, y, value)
      }
    }
  })
